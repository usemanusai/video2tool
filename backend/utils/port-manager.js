/**
 * Port management utility for the backend server
...
function terminateProcess(pid) {
  try {
    ...
    logger.error(`Failed to terminate process with PID ${pid}: ${error.message}`);
    return false;
  }
}
...
function freeUpPort(port) {
  ...
  if (!isPortInUse(port)) {
    logger.info(`Port ${port} is not in use.`);
    return true;
  }
  
  const pid = getProcessIdForPort(port);
  if (!pid) {
    logger.warn(`Could not find process ID for port ${port}.`);
    return false;
  }
  
  logger.warn(`Port ${port} is in use by process with PID ${pid}. Attempting to terminate...`);
  
  const terminated = terminateProcess(pid);
  if (terminated) {
    logger.info(`Successfully terminated process with PID ${pid} using port ${port}.`);
    return true;
  } else {
