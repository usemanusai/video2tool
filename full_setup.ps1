...
function Write-Log {
    ...
    try {
        ...
        # If we can't write to the log file, just continue without logging to file
        # This prevents the script from crashing if the log file is locked
    }
}
...
function Test-SupabaseConnection {
    ...
    Write-Log "Testing Supabase connection"

    if (-not $Config.BackendConfig.SupabaseUrl -or -not $Config.BackendConfig.SupabaseKey) {
        Write-Log "Supabase configuration is missing" -Level "ERROR"
        Write-Host "Error: Supabase URL or API key is not configured." -ForegroundColor Red
        Write-Host "Please configure Supabase settings first." -ForegroundColor Yellow
        return $false
    }

    try {
        $headers = @{
            "apikey" = $Config.BackendConfig.SupabaseKey
            "Authorization" = "Bearer " + $Config.BackendConfig.SupabaseKey
        }

        Write-Log "Sending test request to Supabase"
        $response = Invoke-WebRequest -Uri $Config.BackendConfig.SupabaseUrl -Headers $headers -Method GET

        if ($response.StatusCode -eq 200) {
            Write-Log "Supabase connection test successful"
            Write-Host "Successfully connected to Supabase!" -ForegroundColor Green
            return $true
        } else {
            Write-Log "Supabase connection test failed with status code: $($response.StatusCode)" -Level "ERROR"
            Write-Host "Failed to connect to Supabase. Status code: $($response.StatusCode)" -ForegroundColor Red
            return $false
        }
    }
    catch {
        Write-Log "Error testing Supabase connection: $_" -Level "ERROR"
        Write-Host "Error connecting to Supabase: $_" -ForegroundColor Red
        return $false
    }
}
...
function Test-SupabaseConnection {
    ...
    if (-not $Config.BackendConfig.SupabaseUrl -or -not $Config.BackendConfig.SupabaseKey) {
        Write-Host "Supabase URL or Key not configured. Please configure them first." -ForegroundColor Yellow
        return $false
    }

    try {
        # Prepare the request
        $url = "$($Config.BackendConfig.SupabaseUrl)/rest/v1/"
        $headers = @{
            "apikey" = $Config.BackendConfig.SupabaseKey
            "Authorization" = "Bearer $($Config.BackendConfig.SupabaseKey)"
        }
        ...
        # Test the connection
        Write-Log "Connecting to Supabase at $url"
        $response = Invoke-WebRequest -Uri $url -Headers $headers -Method GET -ErrorAction Stop

        if ($response.StatusCode -eq 200) {
            Write-Host "Successfully connected to Supabase!" -ForegroundColor Green
            Write-Log "Supabase connection successful"
            return $true
        } else {
            Write-Host "Connection to Supabase failed with status code: $($response.StatusCode)" -ForegroundColor Red
            Write-Log "Supabase connection failed with status code: $($response.StatusCode)" -Level "ERROR"
            return $false
        }
    }
    catch {
        Write-Host "Error connecting to Supabase: $_" -ForegroundColor Red
        Write-Log "Error connecting to Supabase: $_" -Level "ERROR"
        return $false
    }
}
...
function Show-MainMenu {
    ...
    while ($true) {
        ...
        Write-Host "17. Test Supabase Connection"
        Write-Host ""
        Write-Host "0. Exit"
        Write-Host ""
        ...
        switch ($choice) {
            ...
            "17" {
                ...
                Test-SupabaseConnection -Config $Config
                Read-Host "`nPress Enter to continue"
            }
            "0" {
                return
